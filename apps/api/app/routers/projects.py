...
class ProjectIn(BaseModel):
    ...
    materials: str | None = Field(None, max_length=2000)
...
def list_projects(user: AdminUser = Depends(require_admin)):
    q = db.table("projects").select(
        "id,current_phase,address,location_link,installation_notes,materials,system_size_kwp,contract_value,paid_amount,payment_method,down_payment,installments,"
...
async def reschedule_notice(
    ...
    # Atualiza a data da fase 6 (Instalação agendada) se foi pedido
    if payload.new_scheduled_date:
        db.table("project_phases").update({"scheduled_date": payload.new_scheduled_date}) \
            .eq("project_id", project_id).eq("phase_number", 6).execute()
