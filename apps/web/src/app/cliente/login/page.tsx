...
export default function ClienteLogin() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data }) => {
      if (data.session) router.replace("/cliente");
    });
  }, [router]);
    ...
    <main className="min-h-screen flex items-center justify-center p-6 bg-invictus">
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-card p-8">
        <div className="flex flex-col items-center mb-6">
          <Image src="/Invictus-Logo-site.png" alt="Invictus" width={80} height={80} className="object-contain" />
          <h1 className="mt-4 font-bold text-invictus-deep text-xl">Meu portal</h1>
          <p className="text-xs text-slate-500 mt-1">Acompanhe seu projeto solar</p>
        </div>
        ...
        <p className="text-[11px] text-slate-500 mt-4 text-center">
          Recebeu suas credenciais por WhatsApp? Use o e-mail e senha informados.
        </p>
      </div>
    </main>
  );
